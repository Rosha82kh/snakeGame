...
public class Snake extends JPanel {
    private static final Color c=new Color(115,162,75);
    private static final int start=250;
    private static final int speed=25;
    private ArrayList<main.Rectangle>body;
    ...
    public void addPart(){
        main.Rectangle last=this.body.get(this.body.size()-1);
        switch(this.direction){
            case "right"-> this.body.add(new main.Rectangle(last.getPosx()- rec_width,last.getPosy()));
            case "left"->this.body.add(new main.Rectangle(last.getPosx()+ rec_width,last.getPosy()));
            case "up"->this.body.add(new Rectangle(last.getPosx(),last.getPosy()+rec_width));
            case "down"->this.body.add(new main.Rectangle(last.getPosx(),last.getPosy()-rec_width));
        }

    }
public void checkColission(){
        main.Rectangle r3=this.body.get(0);
        for (int i=1;i<this.body.size();i++){
            main.Rectangle r2= this.body.get(i);
            if(r3.intersects(r2)){
...
public void moveSnake(){
        ArrayList<main.Rectangle> newlist=new ArrayList<>();
...
newlist.add(head);
    ...
    private void drawSnake(Graphics g){
        ...
        if (this.apple!= null){
            ...
            g2d.drawRect(this.apple.getPosx(),this.apple.getPosy(), rec_width , main.Rectangle.rec_height);
            g2d.fillRect(this.apple.getPosx(),this.apple.getPosy(), rec_width, main.Rectangle.rec_height);

         }
g2d.setPaint(Color.blue);
        for(main.Rectangle rec:this.body){
            g2d.drawRect(rec.getPosx(),rec.getPosy(), rec_width, main.Rectangle.rec_height);
            g2d.fillRect(rec.getPosx(),rec.getPosy(), rec_width, Rectangle.rec_height);
        }
